...
inline void calculate_tangent() {
    ...
    for (int d = 0; d < ITERATIONS; d++) {
        ...
        rounding_bias = sfpi::vFloat(__builtin_rvtt_sfpxloadi(0x4b40, 0));
        j = sfpi::vFloat(__builtin_rvtt_sfpmad(v.get(), inv_pio2.get(), rounding_bias.get(), sfpi::SFPMAD_MOD1_OFFSET_NONE));
...
inline void calculate_sine() {
    ...
    for (int d = 0; d < ITERATIONS; d++) {
        ...
        // Workaround for SFPI's insistence on generating SFPADDI+SFPMUL instead of SFPLOADI+SFPMAD here.
        sfpi::vFloat rounding_bias;
        rounding_bias = sfpi::vFloat(__builtin_rvtt_sfpxloadi(0x4b40, 0));  // 1.5*2^23
        sfpi::vFloat inv_pi = __builtin_rvtt_sfpreadlreg(sfpi::vConstFloatPrgm2.get());

        // Compute j = round(v / PI).
        // First, j = v * (1 / PI) + 1.5*2^23 shifts the mantissa bits to give round-to-nearest-even.
        sfpi::vFloat j;
        j = sfpi::vFloat(__builtin_rvtt_sfpmad(v.get(), inv_pi.get(), rounding_bias.get(), SFPMAD_MOD1_OFFSET_NONE));
...
inline void calculate_cosine() {
    ...
    for (int d = 0; d < ITERATIONS; d++) {
        ...
        sfpi::vFloat half;
        half = sfpi::vFloat(__builtin_rvtt_sfpxloadi(0x3f00, 0));  // 0.5
        sfpi::vFloat inv_pi = __builtin_rvtt_sfpreadlreg(sfpi::vConstFloatPrgm2.get());
        sfpi::vFloat one = __builtin_rvtt_sfpreadlreg(sfpi::vConst1.get());
        sfpi::vFloat neg_one = __builtin_rvtt_sfpreadlreg(sfpi::vConstNeg1.get());
        ...
        sfpi::vFloat j;
        j = sfpi::vFloat(__builtin_rvtt_sfpmad(v.get(), inv_pi.get(), half.get(), SFPMAD_MOD1_OFFSET_NONE));

        // sfpi::vFloat rounding_bias;
        // rounding_bias = sfpi::vFloat(__builtin_rvtt_sfpxloadi(0x4b40, 0));  // 1.5*2^23
        // j = sfpi::vFloat(__builtin_rvtt_sfpmad(v.get(), one, rounding_bias.get(), SFPMAD_MOD1_OFFSET_NONE));
        ...
        sfpi::vFloat two;
        two = sfpi::vFloat(__builtin_rvtt_sfpxloadi(0x4000, 0));  // 2.0
        j = sfpi::vFloat(__builtin_rvtt_sfpmad(j.get(), two.get(), neg_one.get(), SFPMAD_MOD1_OFFSET_NONE));
